...
export function cmdContext(parsed: Parsed): number {
  ...
  if (name === undefined) {
    ...
    if (parsed.flags.variant !== undefined) return usageError(`${command}: --variant requires a component name`);
    if (parsed.flags.node !== undefined) return usageError(`${command}: --node requires a component name`);
  ...
  const out = stableJsonFile(view === "agent" ? projectAgentContext(context) : context);
...
function fatalMessage(command: string, error: Extract<ReturnType<typeof buildDesignContext>, { ok: false }>["error"]): string {
  if (error.kind === "collision") {
    ...
    for (const id of error.candidates) counts.set(id, (counts.get(id) ?? 0) + 1);
    const hasUniqueId = [...counts.values()].some((count) => count === 1);
    const duplicateId = [...counts].find(([, count]) => count > 1)?.[0];
    if (duplicateId !== undefined && !hasUniqueId) {
      return `${command}: NAME_COLLISION "${error.name}" matches ${error.candidates.length} component sets `
