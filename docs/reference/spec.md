...
| S3 visual difference | For each expected variant with a reference, align the 2x sample PNG and 2x rendered screenshot to the same box and measure the pixelmatch mismatch ratio at threshold 0.1. The gated value is the worst variant's ratio; the mean is recorded alongside it. A missing PNG produces `null`. | At most 0.05. |
...
S3 carries the same kind of status. `not-applicable` means no expected variant records a `renderPng`, and no browser is launched. `measured` means at least one referenced variant was rendered and compared; a variant the output did not render is a coverage miss, not an S3 error, and S3 is measured over the intersection. `error` withholds the gated `s3` and makes `overall` false, with a code such as `NO_SNAPSHOT_COMPONENT`, `AMBIGUOUS_SNAPSHOT_COMPONENT`, `REFERENCE_FILE_MISSING`, `FILENAME_RULE_MISMATCH`, `PNG_DECODE_FAILED`, `RENDER_FAILED`, `NO_COMPARABLE_VARIANT`, or `DOM_DISAGREEMENT`. The reference PNG is resolved through the snapshot's own `renderPng`, cross-checked against the importer's node-id filename rule. When the render page's `[data-variant]` list disagrees with the tokenizer, both S3 and coverage take `DOM_DISAGREEMENT`. The gated aggregate is the worst variant's ratio (`max`); the mean is recorded alongside it as a diagnostic only. The worst variant is the gated value because the 0.05 threshold is a per-variant tolerance: gating on the mean would loosen the effective tolerance by the variant count and let one fully broken variant of many pass.
...
## What works
| Sample class | Input | Input variant | S1 | S2 | S3 | Coverage | Input tokens p50 (cache-write rows) | Cost p50 (all sent rows) | Latency p50 (all sent rows) | n sent | n cache-write |
## What does not work
