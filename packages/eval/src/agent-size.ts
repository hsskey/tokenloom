...
export function agentSizeOf(sample: string, context: DesignContextT): AgentSize {
  ...
  const agentBytes = bytes(projectAgentContext(context));
  const reductionPct = ((canonicalBytes - agentBytes) / canonicalBytes) * 100;
  return { sample, canonicalBytes, agentBytes, reductionPct };
}
