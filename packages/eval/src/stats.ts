...
export function outputBasis(runs: EvalRun[], model: string, promptHash: string): OutputBasis {
  // Match the matrix alias: once a resolved ID is recorded, `model` no longer equals it (SPEC 9.1).
...
// Different model provenance, harness state, or reference lock must never share a table (SPEC 9.6).
