...
export function cmdTokensBuild(parsed: Parsed): number {
  ...
  for (const w of result.warnings) process.stderr.write(`${w.code} ${w.nodeId} ${w.detail}\n`);
  process.stderr.write(`tokens: ${written.length} files, ${result.tokens} tokens, ${result.warnings.length} warnings\n`);
  if (flagBool(parsed, "json")) {
    process.stdout.write(stableJsonFile({ files: written, tokens: result.tokens, warnings: result.warnings, at: Date.now() }));
  }
